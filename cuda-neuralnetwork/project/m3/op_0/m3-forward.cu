                                  ...
                                  int Batch, int Map_out, int Channel, int Height, int Width, int K)
{
    ...
    */
    ...
    __shared__ float shared_mask_tile[TILE_SIZE][TILE_SIZE];
    ...
    for(int chunk = 0; chunk < numChunks; chunk++){
        int mask_col = chunk * TILE_SIZE + threadIdx.x;
        if (rowIdx < Map_out && mask_col < Acols){
            shared_mask_tile[threadIdx.y][threadIdx.x] = other_mask[rowIdx * Acols + mask_col]; 
        } 
        else{
            shared_mask_tile[threadIdx.y][threadIdx.x] = 0;
        }
        ...
        //     printf("chunk %d, mask_tile[0][0]=%f\n", chunk,
        //            shared_mask_tile[0][0]);
        // }

        for(int k = 0; k < TILE_SIZE; ++k){
            total += shared_mask_tile[threadIdx.y][k] * tileB[k][threadIdx.x];
        }
...
__host__ void GPUInterface::conv_forward_gpu_prolog(const float *host_output, const float *host_input, const float *host_mask, float **device_output_ptr, float **device_input_ptr, float **device_mask_ptr, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    if(mask_count > MASK_SIZE){
        std::cerr << "Error: Mask size exceeds constant memory limit (" << mask_count << " floats)." << std::endl;
        exit(1);
    }
    cudaMemcpyToSymbol(other_mask, host_mask, mask_count * sizeof(float));
